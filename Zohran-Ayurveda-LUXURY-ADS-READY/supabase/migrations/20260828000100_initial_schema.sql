...
-- Luxury commerce layer
alter table public.orders add column if not exists user_id uuid references auth.users(id) on delete set null;
alter table public.orders add column if not exists subtotal_paise integer not null default 0;
...
create table if not exists public.customer_profiles (
  ...
  full_name text,
  phone text,
  loyalty_tier text not null default 'member' check (loyalty_tier in ('member','premium','vip')),
...
create index if not exists coupons_active_idx on public.coupons(active, starts_at, ends_at);
...
create policy "Users can read own profile" on public.customer_profiles for select to authenticated using (auth.uid()=user_id);
drop policy if exists "Users can update own profile" on public.customer_profiles;
create policy "Users can update own profile" on public.customer_profiles for all to authenticated using (auth.uid()=user_id) with check (auth.uid()=user_id);
drop policy if exists "Admins can manage profiles" on public.customer_profiles;
create policy "Admins can manage profiles" on public.customer_profiles for all to authenticated using ((auth.jwt()->'app_metadata'->>'role')='admin') with check ((auth.jwt()->'app_metadata'->>'role')='admin');

drop policy if exists "Public can view active coupons" on public.coupons;
create policy "Public can view active coupons" on public.coupons for select using (active=true);
drop policy if exists "Admins can manage coupons" on public.coupons;
create policy "Admins can manage coupons" on public.coupons for all to authenticated using ((auth.jwt()->'app_metadata'->>'role')='admin') with check ((auth.jwt()->'app_metadata'->>'role')='admin');
...
create policy "Admins can manage blogs" on public.blog_posts for all to authenticated using ((auth.jwt()->'app_metadata'->>'role')='admin') with check ((auth.jwt()->'app_metadata'->>'role')='admin');

-- Customers can read their own orders/items; admins retain full access.
drop policy if exists "Customers can read own orders" on public.orders;
create policy "Customers can read own orders" on public.orders for select to authenticated using (auth.uid()=user_id);
...
declare v_order public.orders; v_item record; v_product public.products;
begin
 select * into v_order from public.orders where id=p_order_id for update;
 if not found then raise exception 'Order not found'; end if;
 if v_order.payment_status='captured' then return v_order; end if;
 for v_item in select * from public.order_items where order_id=p_order_id loop
  select * into v_product from public.products where id=v_item.product_id for update;
  if not found then raise exception 'Product missing'; end if;
  if v_product.stock_qty < v_item.quantity then
   update public.orders set status='payment_review',payment_status='authorized',razorpay_payment_id=p_payment_id where id=p_order_id returning * into v_order; return v_order;
  end if;
 end loop;
 for v_item in select * from public.order_items where order_id=p_order_id loop update public.products set stock_qty=stock_qty-v_item.quantity where id=v_item.product_id; end loop;
 update public.orders set status='paid',payment_status='captured',razorpay_payment_id=p_payment_id where id=p_order_id returning * into v_order;
 if v_order.coupon_code is not null then update public.coupons set used_count=used_count+1 where code=v_order.coupon_code; end if;
 if v_order.user_id is not null then
   insert into public.customer_profiles(user_id,lifetime_spend_paise) values(v_order.user_id,v_order.total_paise)
   on conflict(user_id) do update set lifetime_spend_paise=public.customer_profiles.lifetime_spend_paise+excluded.lifetime_spend_paise;
 end if;
 return v_order;
end; $$;
revoke all on function public.mark_order_paid(uuid,text) from public,anon,authenticated;
